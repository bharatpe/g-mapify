...
class GMap extends React.PureComponent {
  constructor(props) {
    ...
    this.mapElemRef = null;
    this.defaultSearchPlace = null;
    this.mapLastPosition = {};
    this.addressInput = "";
    this.state = {
      searchResults: [],
      showMapSearch: false,
    ...
    this.searchByQueryDebounce = debounce(this.searchByQueryDebounce, this.props.debounceTime);
  ...
  insertMapScript() {
    ...
    if (!isGMapScriptAdded) {
      ...
      const librariesOptions = this.props.mapOptions.libraries || GMap.defaultMapOptions.libraries;
  ...
  createMapInstance(lat, lng) {
    ...
    if (this.mapElemRef.current) {
      this.mapInstance = new window.google.maps.Map(this.mapElemRef.current, { ...center,
  ...
  addEvents() {
    if (this.mapInstance) {
      this.mapInstance.addListener("dragend", () => {
        this.setMapPosition(this.mapInstance.center.lat(), this.mapInstance.center.lng());
      });
  ...
  setMapPosition(lat, lng) {
    ...
    if (this.mapLastPosition.lat !== position.lat && this.mapLastPosition.lng !== position.lng) {
      ...
      this.addSearchBox();
      this.getAddressFromLatLong(position).then((data, status) => {
  ...
  searchByQuery(query) {
    return new Promise((resolve, reject) => {
      ...
      const service = new window.google.maps.places.PlacesService(this.mapInstance);
  ...
  searchByQueryDebounce(query) {
    ...
    this.searchByQuery(query).then(results => {
  ...
  selectMapItem(event) {
    ...
    if (closestLiElem.hasAttribute("index")) {
      const selectedVal = this.state.searchResults[Number(closestLiElem.getAttribute("index"))];
      this.setState({
        showMapSearch: false
      });
